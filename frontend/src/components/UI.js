...
export function Button({ children, onClick, disabled, variant = "primary", style = {}, fullWidth }) {
  const variants = {
...
export function StatCard({ label, value, sub, color = C.text, bg = C.card }) {
  return (
    <div style={{
...
export function SectionHeader({ title, sub, right }) {
  return (
    <div style={{ display: "flex", justifyContent: "space-between", alignItems: right ? "center" : "flex-start", marginBottom: 16 }}>
      <div>
